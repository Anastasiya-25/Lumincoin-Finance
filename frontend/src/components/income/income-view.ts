...
export class IncomeView {
    ...
    constructor(route: Router) {
        ...
        if (confirmBtn) {
            confirmBtn.addEventListener('click', (): Promise<void> => this.incomeDelete());
        }
    }

    private async getIncomes(): Promise<void> {
        const result: ResultResponseType<IncomeExpenseResponseType[]> = await HttpUtils.request('/categories/income');
    ...
    private showIncomes(incomes: IncomeExpenseResponseType[]): void {
        const container: HTMLElement | null = document.getElementById('cards-container');
        if (container) {
            container.innerHTML = '';
            incomes.forEach((income: IncomeExpenseResponseType): void=> {
    ...
    private async incomeDelete(): Promise<void> {
        if (this.deleteId) {
            const result: ResultResponseType<IncomeExpenseResponseType[]> = await HttpUtils.request('/categories/income/' + this.deleteId, 'DELETE', true);
